...
export function App({ organization, session }) {
  ...
  const preflight = {
    classCapacity: Object.values(classLoads).some((load) => load > weeklyCapacity),
    teacherCapacity: Object.values(teacherLoads).some((load) => load > weeklyCapacity),
    ...
    coverage: !classes.length || !subjects.length || !teachers.length || !assignments.length,
  };
        ...
        <div className="page-wrap">
          {step === -1 ? <DashboardStep school={school} classes={classes} totalSections={totalSections} subjects={subjects} teachers={teachers} assignments={assignments} blockers={blockers} generated={generated} onSetup={() => setStep(0)} /> : <>
            ...
            <section className="step-content">
              ...
              {step === 3 && <SubjectsStep subjects={subjects} setSubjects={setSubjects} classes={classes} frequencies={frequencies} updateFrequency={updateFrequency} totalFrequency={totalFrequency} weeklyCapacity={days * periods} />}
              ...
              {step === 6 && <ReviewStep school={school} schoolId={schoolId} classes={classes} totalSections={totalSections} days={days} periods={periods} subjects={subjects} teachers={teachers} assignments={assignments} blockers={blockers} preflight={preflight} setStep={setStep} notify={notify} generated={generated} onGenerate={generate} />}
            </section>
...
function SubjectsStep({ subjects, setSubjects, classes, frequencies, updateFrequency, totalFrequency, weeklyCapacity }) { const [newSubject, setNewSubject] = useState(''); const [selectedClass, setSelectedClass] = useState(classes[0]); const addSubject = () => { if (newSubject.trim() && !subjects.includes(newSubject.trim())) { setSubjects([...subjects, newSubject.trim()]); setNewSubject(''); } }; const capacity = weeklyCapacity || 0; const totalForClass = Object.values(frequencies[selectedClass] || {}).reduce((a, b) => a + b, 0); return <div className="stack"><Card icon={BookOpen} title="Subject library" description="Keep one master list. Subject colors stay consistent across every timetable view." accent="blue"><div className="subject-grid">{subjects.map((subject) => <div className="subject-card" key={subject}><span className="subject-swatch" style={{ background: subjectColors[subject] || '#6b7fc1' }}>{subject[0]}</span><div><strong>{subject}</strong><small>{subject === 'English' || subject === 'Mathematics' ? 'Core subject' : 'Available to assign'}</small></div><button onClick={() => setSubjects(subjects.filter((item) => item !== subject))}><Trash2 size={15} /></button></div>)}</div><div className="inline-input"><input value={newSubject} onChange={(e) => setNewSubject(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && addSubject()} placeholder="e.g. Robotics, Music" /><button className="secondary-button" onClick={addSubject}><Plus size={16} /> Add subject</button></div></Card><Card icon={SlidersHorizontal} title="Frequency by class" description="How many periods per week should each subject run in each class?" accent="yellow"><div className="class-tabs">{classes.map((name) => { const total = Object.values(frequencies[name] || {}).reduce((a, b) => a + b, 0); return <button className={selectedClass === name ? 'active' : ''} key={name} onClick={() => setSelectedClass(name)}>{name}<small>{total}/{capacity}</small></button>; })}</div><div className="coverage-line"><span>{selectedClass || 'Select a class'}</span><b>{totalForClass} / {capacity} periods per week</b></div><div className="frequency-list">{subjects.map((subject) => <div key={subject}><span><i style={{ background: subjectColors[subject] || '#6b7fc1' }} />{subject}</span><div className="stepper-control compact"><button onClick={() => updateFrequency(selectedClass, subject, -1)} disabled={!selectedClass || totalForClass <= 0}>−</button><b>{frequencies[selectedClass]?.[subject] || 0}</b><button onClick={() => updateFrequency(selectedClass, subject, 1)} disabled={!selectedClass || totalForClass >= capacity}>+</button></div></div>)}</div></Card></div>; }

function TeachersStep({ teachers, setTeachers, assignments, setAssignments, showAddTeacher, setShowAddTeacher, notify }) {
  const [draft, setDraft] = useState({ name: '', email: '' });
  const [editing, setEditing] = useState(null);
  const addTeacher = () => {
    if (!draft.name.trim()) return;
    setTeachers([...teachers, { ...draft, initials: draft.name.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase(), assignments: 0 }]);
    setDraft({ name: '', email: '' }); setShowAddTeacher(false); notify('Teacher added');
  };
  const startEdit = (teacher) => { setEditing(teacher.name); setDraft({ name: teacher.name, email: teacher.email || '' }); };
  const saveEdit = () => {
    if (!draft.name.trim() || !editing) return;
    const nextName = draft.name.trim();
    setTeachers(teachers.map((teacher) => teacher.name === editing ? { ...teacher, name: nextName, email: draft.email, initials: nextName.split(' ').map((part) => part[0]).join('').slice(0, 2).toUpperCase() } : teacher));
    setAssignments(assignments.map((item) => item.teacher === editing ? { ...item, teacher: nextName } : item));
    setEditing(null); setDraft({ name: '', email: '' }); notify('Teacher updated');
  };
  const deleteTeacher = (teacher) => {
    if (!window.confirm(`Delete ${teacher.name}? Their assignments will also be removed.`)) return;
  ...
  return <div className="stack"><div className="summary-strip"><Metric label="Teachers" value={teachers.length} icon={Users} /><Metric label="Class teachers" value="0" icon={GraduationCap} /><Metric label="Assignments" value={assignments.length} icon={BookOpen} /><Metric label="Periods / week" value={assignments.reduce((sum, item) => sum + item.periods, 0)} icon={Clock3} /></div><Card icon={Users} title="Teaching team" description="Add staff and the subjects/classes they teach." accent="blue"><div className="card-actions"><button className="secondary-button" onClick={() => { setEditing(null); setDraft({ name: '', email: '' }); setShowAddTeacher(true); }}><Plus size={16} /> Add teacher</button></div>{(showAddTeacher || editing) && <div className="add-row"><input placeholder="Teacher name" value={draft.name} onChange={(e) => setDraft({ ...draft, name: e.target.value })} /><input placeholder="Email (optional)" value={draft.email} onChange={(e) => setDraft({ ...draft, email: e.target.value })} />{editing ? <button className="primary-button small" onClick={saveEdit}>Save changes</button> : <button className="primary-button small" onClick={addTeacher}>Save</button>}<button className="ghost-button small" onClick={() => { setEditing(null); setShowAddTeacher(false); setDraft({ name: '', email: '' }); }}>Cancel</button></div>}<div className="search-row"><input placeholder="Search by name or email" /><div className="filter-pills"><button className="active">All</button><button>Class teachers</button><button>Coordinators</button></div></div><div className="teacher-list">{teachers.map((teacher) => <div className="teacher-row" key={teacher.email || teacher.name}><div className="teacher-avatar">{teacher.initials}</div><div className="teacher-info"><strong>{teacher.name}</strong><small>{teacher.email || 'No email added'}</small></div><span className="teacher-periods">{teacher.assignments} assignments</span><button className="icon-button" title="Edit teacher" onClick={() => startEdit(teacher)}><Settings2 size={16} /></button><button className="icon-button" title="Delete teacher" onClick={() => deleteTeacher(teacher)}><Trash2 size={16} /></button></div>)}</div></Card></div>;
}
...
function ReviewStep({ school, schoolId, classes, totalSections, days, periods, subjects, teachers, assignments, blockers, preflight, setStep, notify, generated, onGenerate }) { const cards = [{ label: 'School', value: school.name, sub: school.year, icon: Building2, step: 0, accent: 'blue' }, { label: 'Classes', value: `${classes.length} classes`, sub: `${totalSections} sections`, icon: GraduationCap, step: 1, accent: 'coral' }, { label: 'Periods', value: `${periods} regular`, sub: `${days}-day week`, icon: Clock3, step: 2, accent: 'yellow' }, { label: 'Subjects', value: `${subjects.length} subjects`, sub: 'Library ready', icon: BookOpen, step: 3, accent: 'blue' }, { label: 'Teachers', value: `${teachers.length} teachers`, sub: `${assignments.length} assignments`, icon: Users, step: 4, accent: 'coral' }, { label: 'Coverage', value: blockers ? `${blockers} blockers` : 'Ready to generate', sub: blockers ? 'Fix setup issues' : 'All essentials covered', icon: blockers ? AlertTriangle : CheckCircle2, step: 5, accent: blockers ? 'red' : 'green' }]; const status = (issue) => issue ? 'Review' : 'Ready'; return <div className="stack"><div className={`review-banner ${blockers ? 'warning' : 'ready'}`}><div className="review-banner-icon">{blockers ? <AlertTriangle size={22} /> : <CheckCircle2 size={22} />}</div><div><h2>{blockers ? 'A few things need attention' : 'Your setup is ready'}</h2><p>{blockers ? 'Some constraints cannot fit in the current weekly capacity. Review the flagged checks below.' : 'All essentials are covered. You can generate a first timetable now.'}</p></div></div><div className="review-grid">{cards.map((card) => { const Icon = card.icon; return <button className="review-card" key={card.label} onClick={() => setStep(card.step)}><div className={`card-icon ${card.accent}`}><Icon size={18} /></div><div className="mini-label">{card.label}</div><strong>{card.value}</strong><small>{card.sub}</small><ChevronRight size={16} className="review-arrow" /></button>; })}</div><Card icon={AlertTriangle} title="Pre-flight checks" description="These checks explain why the solver may fail before generation starts." accent="red"><div className="check-list"><div><CheckCircle2 size={17} /><span>No class double-booking</span><b>{status(preflight.classCapacity)}</b></div><div><CheckCircle2 size={17} /><span>Teacher availability</span><b>{status(preflight.teacherCapacity)}</b></div><div><CheckCircle2 size={17} /><span>Subject frequency fits working days</span><b>{status(preflight.frequency)}</b></div><div><CheckCircle2 size={17} /><span>Subject and teacher coverage</span><b>{status(preflight.coverage)}</b></div></div><button className="primary-button generate-button" onClick={onGenerate}><Sparkles size={17} /> Generate timetable</button>{generated && <div className={`generation-result ${generated.status === 'INFEASIBLE' ? 'failed' : ''}`}><strong>{generated.status === 'INFEASIBLE' ? 'Generation needs attention' : `Generated ${generated.entries.length} periods`}</strong><span>{generated.status === 'INFEASIBLE' ? generated.diagnostics?.[0] : `Solver finished in ${generated.solveSeconds || '—'} seconds.`}</span></div>}</Card>{generated?.entries?.length > 0 && <><SchedulePreview entries={generated.entries} /><AbsencePanel schoolId={schoolId} teachers={teachers} notify={notify} /></>}</div>; }

function SchedulePreview({ entries }) {
  const classNames = [...new Set(entries.map((entry) => entry.className))];
  const [selectedClass, setSelectedClass] = useState(classNames[0] || '');
  const classEntries = entries.filter((entry) => entry.className === selectedClass);
  const dayOrder = ['Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'];
  const days = dayOrder.filter((day) => classEntries.some((entry) => entry.day === day));
  const periods = [...new Set(classEntries.map((entry) => entry.period))].sort((a, b) => a - b);
  return <Card icon={CalendarDays} title="Generated timetable" description="Each class has its own timetable. Choose a class to view its weekly schedule." accent="green">
    <div className="schedule-meta"><span><CheckCircle2 size={14} /> {entries.length} periods placed</span><span>{classNames.length} class timetables</span></div>
    <div className="class-tabs schedule-class-tabs">{classNames.map((name) => <button className={selectedClass === name ? 'active' : ''} key={name} onClick={() => setSelectedClass(name)}>{name}</button>)}</div>
    <div className="schedule-table"><div className="schedule-head"><span>Period</span>{days.map((day) => <span key={day}>{day.slice(0, 3)}</span>)}</div>{periods.map((period) => <div className="schedule-row" key={period}><strong>P{period}</strong>{days.map((day) => { const entry = classEntries.find((item) => item.day === day && item.period === period); return <div className="schedule-cell" key={`${day}-${period}`}>{entry ? <><b style={{ color: subjectColors[entry.subject] || '#3157d5' }}>{entry.subject}</b><small>{entry.teacher}</small></> : <span>—</span>}</div>; })}</div>)}</div>
  </Card>;
}
